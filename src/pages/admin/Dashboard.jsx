...
export default function Dashboard() {
  ...
  const modules = [
    ...
    { title: 'Usuários', icon: Users, path: '/admin/employees' },
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/80">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs font-bold text-[#ff8b00] bg-orange-50 px-2.5 py-1 rounded-full border border-orange-200">
              86% Concluído
            </span>
          </div>
          <div className="w-full bg-slate-100 rounded-full h-2.5 mb-5 overflow-hidden">
            <div className="bg-[#ff8b00] h-2.5 rounded-full transition-all duration-500" style={{ width: '86%' }}></div>
          </div>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 rounded-xl bg-slate-50 border border-slate-200/60">
            ...
            <button className="bg-[#1a2c6a] hover:bg-[#121f4c] text-white px-5 py-2 rounded-xl text-sm font-semibold transition flex items-center gap-2 shadow-sm">
              Continuar <ArrowRight size={16} />
            </button>
          </div>
        </div>

        {/* Grid de Cards de Módulos (Estilo Coalize) */}
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {modules.map((m, idx) => {
              ...
              return (
                <div 
                  key={idx} 
                  className="relative group bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm hover:shadow-md hover:border-[#ff8b00] transition cursor-pointer flex flex-col items-center text-center"
                >
                  {m.isNew && (
                    <span className="absolute top-3 right-3 text-[10px] font-extrabold bg-[#ff8b00] text-white px-2 py-0.5 rounded-full uppercase tracking-wider">
                      Novo
                    </span>
                  )}
                  <div className="w-12 h-12 rounded-2xl bg-slate-50 group-hover:bg-orange-50 text-[#1a2c6a] group-hover:text-[#ff8b00] flex items-center justify-center mb-3 transition">
                    <Icon size={24} />
                  </div>
